...
# Analyze root cause and prevention steps using local GPT
def analyze_issue_context(issue_key, summary, comments_dict):
    start_prompt = time.time()
    prompt = (
        f"You are a support assistant who has to summerize the cause of the issue based on the ticket details below. Do not use proper nouns. Do not use emojis. Just give the cause of the ticket if possible. If not possible, just tell 'No cause/resolution found. Please create a ticket'. Based on the details below, provide a concise cause/resolution (max 200 characters) "
        ...
        f"The likely cause of this issue.\n"
        f"Keep the answer under 400 characters total."
    )
    ...
    if response.status_code == 200:
        ...
        return "⚠️ Failed to analyze this issue."
